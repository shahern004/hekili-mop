...
-- Debug: Print when abilities are registered and copy shared abilities
print("DEBUG: Survival Hunter abilities registered")

-- Copy shared abilities from the 'all' spec (ID 0) to this spec
if class.specs[0] and class.specs[0].abilities then
    local sharedCount = 0
    print("DEBUG: Copying shared abilities from 'all' spec to Survival Hunter...")
    
    for abilityName, abilityData in pairs(class.specs[0].abilities) do
        if not spec.abilities[abilityName] then
            spec.abilities[abilityName] = abilityData
            -- Also add to class abilities if not present
            if not class.abilities[abilityName] then
                class.abilities[abilityName] = abilityData
            end
            sharedCount = sharedCount + 1
        end
    end
    
    print("DEBUG: Copied", sharedCount, "shared abilities to Survival Hunter spec")
else
    print("DEBUG: No 'all' spec found or no shared abilities available")
end
