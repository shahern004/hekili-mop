...
print("Classes.lua: Starting to load...")

local addon, ns = ...
local Hekili = _G[ addon ]

print("Classes.lua: addon name:", tostring(addon))
print("Classes.lua: Hekili from _G:", tostring(Hekili ~= nil))

local class = Hekili.Class
local state = Hekili.State

-- Store global functions for use in restricted contexts
local type = _G.type

-- Initialize args and default for ability functions
local args = args or {}
local default = "default"

-- Initialize tinker object for MoP Classic compatibility
_G.tinker = _G.tinker or {
    hand = {
        spell = 0,
        item = 0,
        texture = 0
...
local GetItemIcon = ns.GetItemIcon

local strformat = string.format
local insert, wipe = table.insert, table.wipe

local mt_resource = ns.metatables and ns.metatables.mt_resource or nil
...
local HekiliSpecMixin = {
    RegisterResource = function( self, resourceID, regen, model, meta )
        ...
        r.state.regenModel = regen        r.state.meta = meta or {}
        ...
        end        if r.state.regenModel then
            for _, v in pairs( r.state.regenModel ) do
                -- MoP: Skip if v is a function (like ragePerAuto, regenRate)
    ...
    -- MoP: Add RegisterGlyphs function for glyph system compatibility
    RegisterGlyphs = function( self, glyphs )
        if not self.glyphs then
            self.glyphs = {}
        end
        for glyphID, name in pairs( glyphs ) do
            self.glyphs[ glyphID ] = name
            -- Also register by name for easy access
            if type( name ) == "string" then
                local cleanName = name:gsub("Glyph of ", ""):gsub(" ", "_"):lower()
                self.glyphs[ cleanName ] = glyphID
                CommitKey( cleanName )
            end
        end
    end,
    ...
    RegisterPotion = function( self, potion, data )
        ...
        if data.copy then
            if type( data.copy ) == "table" then
                ...
                CommitKey( data.copy )            end
        end
        
        if data.item and data.item > 0 and tonumber(data.item) and tonumber(data.item) > 0 then
            local itemId = tonumber(data.item)
            -- For MoP Classic, try GetItemInfo first to avoid Item API issues
            local name, link = GetItemInfo(itemId)
              if name and link then
                -- Use direct item info
                ...
                -- Fallback to Item API only if GetItemInfo fails and itemId is valid
                if itemId and itemId > 0 and type(itemId) == "number" then
                    local success, potionItem = pcall(function() 
                        -- Additional validation before calling Item API
                        if not itemId or itemId <= 0 or type(itemId) ~= "number" then 
                            return nil 
                        end
                        return Item:CreateFromItemID( itemId ) 
                    end)
                    if success and potionItem and potionItem.IsItemEmpty and not potionItem:IsItemEmpty() then
                        -- Additional safety check before ContinueOnItemLoad
                        if potionItem.ContinueOnItemLoad then
                            potionItem:ContinueOnItemLoad( function()
                                -- Safe item info extraction for MoP compatibility
                                local item_name, item_link
                                
                                local success_name, result_name = pcall(function() return potionItem:GetItemName() end)
                                local success_link, result_link = pcall(function() return potionItem:GetItemLink() end)
                                
                                if success_name and not data.name then data.name = result_name end
                                if success_link and not data.link then data.link = result_link end
                                
                                -- Fallback to GetItemInfo if Item API fails
                                if (not data.name or not data.link) then
                                    local fallback_name, fallback_link = GetItemInfo(data.item)
                                    if not data.name then data.name = fallback_name end
                                    if not data.link then data.link = fallback_link end
                                end
    ...
    RegisterHook = function( self, hook, func, noState )
        if func and type(func) == 'function' and not ( noState == true or hook == "COMBAT_LOG_EVENT_UNFILTERED" and noState == nil ) then
    ...
    RegisterAbility = function( self, ability, data )
        ...
        if a.isItem or item and a.id < 0 then
            ...
            if a.key == "best_mana_potion" then
                ...
                if not a.unlisted then
                    local safeTexture = a.texture or texture or "Interface\\Icons\\inv_misc_questionmark"
                    class.abilityList[ ability ] = "|T" .. safeTexture .. ":0|t " .. (a.link or ability)
                    class.itemList[ a.id ] = "|T" .. safeTexture .. ":0|t " .. (a.link or ability)
                ...
                -- Register the item if it doesn't already exist.                if not class.specs then
                    class.specs = {}
                end
                if class.specs[0] then
                    class.specs[0]:RegisterGear( ability, item )
                end
                if item and item > 0 and tonumber(item) and tonumber(item) > 0 then
                    ...
                    if name and link then
                        -- Use direct item info
                        if not a.name then a.name = name end
                        if not a.link then a.link = link end
                        if not a.texture then a.texture = texture end                    else                        -- Fallback to Item API only if GetItemInfo fails and itemId is valid
                        if itemId and itemId > 0 and type(itemId) == "number" then
                            local success, actionItem = pcall(function() 
                                -- Additional validation before calling Item API
                                if not itemId or itemId <= 0 or type(itemId) ~= "number" then 
                                    return nil 
                                end
                            ...
                            if success and actionItem and actionItem.IsItemEmpty and not actionItem:IsItemEmpty() then
                                -- Additional safety check before ContinueOnItemLoad
                                if actionItem.ContinueOnItemLoad then
                                    actionItem:ContinueOnItemLoad( function( success )
                        -- Safe item info extraction for MoP compatibility
                        local name, link, texture

                        local success_name, result_name = pcall(function() return actionItem:GetItemName() end)
                        local success_link, result_link = pcall(function() return actionItem:GetItemLink() end)
                        local success_texture, result_texture = pcall(function() return actionItem:GetItemIcon() end)

                        if success_name then name = a.name or result_name end
                        if success_link then link = a.link or result_link end
                        if success_texture then texture = a.texture or result_texture end

                        -- Fallback to GetItemInfo if Item API fails
                        if not name or not link or not texture then
                            local fallback_name, fallback_link, _, _, _, _, _, _, _, fallback_texture = GetItemInfo(item)
                            name = name or fallback_name or ability
                            link = link or fallback_link or ability
                            texture = texture or fallback_texture
                        end
                        ...
                        if name then
                            ...
                            if a.item and a.item ~= 158075 then
                                a.itemSpellName, a.itemSpellID = ns.GetItemSpell( a.item )
                            ...
                            if data.items then
                                local addedToItemList = false                                for _, id in ipairs( data.items ) do
                                    -- Extra defensive check for MoP compatibility
                                    if id and type(id) == "number" and id > 0 and tonumber(id) and tonumber(id) > 0 then
                                        -- Store the ID in a local variable to prevent closure issues
                                        local itemId = tonumber(id)
                                        if itemId and itemId > 0 then
                                            -- For MoP Classic, use fallback method instead of Item API for problematic items
                                            local name, link, _, _, _, _, _, _, _, texture = GetItemInfo(itemId)
                                            
                                            if name and link then
                                                -- Use the direct item info instead of the Item API
                                                local safeTexture = a.texture or texture or "Interface\\Icons\\inv_misc_questionmark"
                                                
                                                class.abilities[ name ] = a
                                                if not class.itemList[ itemId ] then
                                                    class.itemList[ itemId ] = "|T" .. safeTexture .. ":0|t " .. link
                                                    addedToItemList = true
                                                end                                            else
                                                -- Fallback: Try Item API only if GetItemInfo fails and itemId is valid
                                                if itemId and itemId > 0 and type(itemId) == "number" then
                                                    local success, copyItem = pcall(function() 
                                                        -- Additional validation before calling Item API
                                                        if not itemId or itemId <= 0 or type(itemId) ~= "number" then 
                                                            return nil 
                                                        end
                                                    ...
                                                    if success and copyItem and copyItem.IsItemEmpty and not copyItem:IsItemEmpty() then
                                                        -- Additional safety check before ContinueOnItemLoad
                                                        if copyItem.ContinueOnItemLoad then
                                                            copyItem:ContinueOnItemLoad( function()
                                                                -- Double-check the item is still valid in the callback
                                                                if not copyItem or not copyItem.IsItemEmpty or copyItem:IsItemEmpty() then return end
                                                -- Safe item info extraction for MoP compatibility
                                                local name, link, texture

                                                local success_name, result_name = pcall(function() return copyItem:GetItemName() end)
                                                local success_link, result_link = pcall(function() return copyItem:GetItemLink() end)
                                                local success_texture, result_texture = pcall(function() return copyItem:GetItemIcon() end)

                                                if success_name then name = result_name end
                                                if success_link then link = result_link end
                                                if success_texture then texture = result_texture end                                                -- Fallback to GetItemInfo if Item API fails
                                                if not name or not link or not texture then
                                                    local fallback_name, fallback_link, _, _, _, _, _, _, _, fallback_texture = GetItemInfo(itemId)
                                                    name = name or fallback_name
                                                    link = link or fallback_link
                                                    texture = texture or fallback_texture
                                                end                                                local safeTexture = a.texture or texture or "Interface\\Icons\\inv_misc_questionmark"
                                                ...
                                                if name then
                                                    ...
                                                    if not class.itemList[ itemId ] then
                                                        class.itemList[ itemId ] = "|T" .. safeTexture .. ":0|t " .. (link or name)
                                                        addedToItemList = true
                                                    end
                                                end
                                            end )
                    ...
                    end -- Close the for _, id in ipairs( data.items ) do loop
                end -- Close the if data.items then block
                        ...
                        return false
                    end -- Close the if actionItem and actionItem.IsItemEmpty check 
                    end -- Close the if itemId and itemId > 0 check )
                )
                end
            end
        end
        ...
        if data.items then
            for _, itemID in ipairs( data.items ) do
                class.itemMap[ itemID ] = ability        end
        end
    ...
    -- MoP: Add RegisterRanges function for range checking compatibility
    RegisterRanges = function( self, ... )
        ...
        for i, ability in ipairs( abilities ) do
            self.ranges[ i ] = ability
            self.ranges[ ability ] = true
        end
    end,
    ...
    end,    RegisterCycle = function( self, func )
        self.cycle = func and type(func) == 'function' and setfenv( func, state ) or func
    end,
    ...
    end,    -- For faster variables.
    RegisterVariable = function( self, key, func )
        CommitKey( key )
        self.variables[ key ] = func and type(func) == 'function' and setfenv( func, state ) or func
    end,RegisterPackSelector = function( self, key, package, name, description, condition )
        insert( self.packSelectors, {
            key = key,
            name = name,
            condition = condition and type(condition) == 'function' and setfenv( condition, state ) or condition
        } )
...
function Hekili:RestoreDefaults()
    ...
    -- Debug: Check if class exists and has packs
    if not class then
        print( "RestoreDefaults: ERROR - class is nil!" )
        return
    end
    
    if not class.packs then
        print( "RestoreDefaults: ERROR - class.packs is nil!" )
        return
    end

    -- Debug: Log the count of class.packs
    local packCount = 0
    for _ in pairs( class.packs ) do packCount = packCount + 1 end
    print( "RestoreDefaults: Found " .. packCount .. " class packs" )
    ...
    for k, v in pairs( class.packs ) do
    ...
    end    if #changed > 0 then
        self:LoadScripts()
        -- self:RefreshOptions()
        print( "RestoreDefaults: Updated " .. #changed .. " packs: " .. table.concat( changed, ", " ) )
    else
        print( "RestoreDefaults: No packs updated" )
    end
        ...
        elseif #changed == 2 then
            ...
            -- MoP: Use legacy timer instead of C_Timer
...
print("Classes.lua: NewSpecialization function defined on Hekili object")
print("Classes.lua: Hekili.NewSpecialization exists:", tostring(Hekili.NewSpecialization ~= nil))
...
all:RegisterAbilities( {
    ...
    potion = {
        ...
        startsCombat = false,
        toggle = "potions",        item = function ()
            local potion = (args and (args.potion or args.name)) or class.potion
        ...
        bagItem = true,        timeToReady = function ()
            local potion = (args and (args.potion or args.name)) or class.potion
            if not potion or potion == "default" then potion = class.potion end
            potion = class.potions[ potion ]
        ...
        usable = function ()
            local pName = (args and (args.potion or args.name)) or class.potion
    ...
    best_mana_potion = {
        ...
        startsCombat = false,
        toggle = "potions",        texture = function()
            local item = class.abilities and class.abilities.best_mana_potion and class.abilities.best_mana_potion.item
            if item and type(item) == "function" then item = item() end
            return GetItemIcon( item )
        end,

        isItem = true,        item = function()
            if not Hekili.PLAYER_ENTERING_WORLD or not state.mana then return 45276 end

            local deficit = state.mana.deficit
        ...
        bagItem = true,        usable = function ()
            local item = class.abilities and class.abilities.best_mana_potion and class.abilities.best_mana_potion.item
            if type(item) == "function" then item = item() end
            return GetItemCount( item ) > 0, "requires a mana deficit and mana_potion in bags"
        end,
        ...
        handler = function ()
            local item = class.abilities and class.abilities.best_mana_potion and class.abilities.best_mana_potion.item
            if type(item) == "function" then item = item() end
            if class.itemMap and class.itemMap[ item ] and class.abilities[ class.itemMap[ item ] ] and class.abilities[ class.itemMap[ item ] ].handler then
                class.abilities[ class.itemMap[ item ] ].handler()
            end
        end,
...
function Hekili:SpecializationChanged()
    print( "SpecializationChanged: Starting..." )
    local currentSpec, currentID, _, currentClass

    if Hekili.IsClassic() then
        currentSpec = 1
        _, currentClass, currentID = UnitClass( "player" )    else
        currentSpec = GetSpecialization()
        currentID = GetSpecializationInfo( currentSpec )        
        
        print( "SpecializationChanged: currentSpec=" .. tostring(currentSpec) .. ", currentID=" .. tostring(currentID) )
        
        if currentID == nil then
            -- MoP: Use legacy timer instead of C_Timer
            print( "SpecializationChanged: currentID is nil, retrying in 0.5s" )
            Hekili:After( 0.5, function () Hekili:SpecializationChanged() end )
            return
        end
    end
    ...
    self.currentSpecOpts = nil    for i, specID in ipairs( specs ) do
        local spec = class.specs[ specID ]
        
        print( "SpecializationChanged: Processing spec " .. specID .. " (found: " .. tostring(spec ~= nil) .. ")" )
        ...
        if spec then
            ...
            end            for k, v in pairs( spec.packs ) do
                if not class.packs[ k ] then 
                    class.packs[ k ] = v 
                    print( "SpecializationChanged: Added pack '" .. k .. "' from spec " .. spec.name )
                end
            end
...
-- Export the class object to Hekili.Class
Hekili.Class = class
